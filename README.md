...
* ダークモードでは、編集画面とWindows側のメニュー、ツールバー、ステータスバー、タイトルバーをまとめて切り替えます。
...
* 本文フォントはゴシックと明朝から選択でき、リッチ、プレビュー、集中モードへ共通適用します。
* 通常のソースモード、コード、URL、数式ソースには、本文フォントの設定にかかわらず等幅フォントを使います。
...
Windowsアプリ版ではMarkdownをUTF-8換算で10MB以下、挿入画像を1ファイル25MB以下に制限します。
...
画面右上のキーボードボタン、またはWindowsアプリ版の「ヘルプ」メニューから割り当てを変更できます。

割り当てにはCtrlと、英字、数字、F1からF12までのいずれかを使い、ShiftとAltを追加できます。

同じキーの重複は保存できません。

変更内容はローカル設定に保存され、設定JSONの書出しと読込みにも含まれます。
...
インライン数式は本文、見出し、表セルで使用できます。

参照リンク定義または表示テキストが空のリンクを含む文書は、元のMarkdownを保持するためリッチ表示が読み取り専用になります。編集にはソースモードを使用します。

リッチ表示へ切り替えただけで本文を編集しなかった場合は、`~~~`、Setext見出し、HTMLエンティティなどの元表記を保持します。隣接するインライン数式と表セル内の数式パイプは、構文を曖昧にしない形へ正規化します。

表セル内の数式に含まれる `|` は列区切りと区別され、リッチ編集とプレビューの両方で数式として扱われます。
