...
|保存と別名保存|WPFとWebView2|保存開始時の本文とリビジョンを固定し、完了までに編集が進んだ場合は未保存状態を維持します。WPFはUTF-8換算の10MB上限を検証し、BOMなしUTF-8で保存します。|
...
読込時は厳密なUTF-8として解釈し、UTF-8のファイルサイズ10MBを上限とします。

保存時は改行をLFへ正規化し、UTF-8換算で10MB以下であることを確認してからBOMなしUTF-8で書き込みます。
...
「新規ウィンドウ」は、内部起動引数 `--new-document` を同じ実行ファイルの別プロセスへ渡します。

新しいプロセスはWebViewの準備完了後に空文書を読み込みます。

「新しいウィンドウで開く」は、選択したMarkdownを同じ実行ファイルの別プロセスへ渡します。

既存ウィンドウの文書パス、未保存状態、画像参照は変更しません。

変更可能なショートカットはCtrlを必須とし、英字、数字、F1からF12までのいずれかへShiftとAltを追加できます。

WebView2とWPFの両側で割り当て形式と重複を検証し、不正な設定をファイル操作へ使いません。
