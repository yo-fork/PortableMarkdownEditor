...
node --check modules\rich-input-controller.js || goto :failed
...
echo.
echo ^> node --check vendor\prosemirror\editor-integration.js
node --check vendor\prosemirror\editor-integration.js || goto :failed

echo.
echo ^> node --check tools\bundle-prosemirror-cjs.mjs
node --check tools\bundle-prosemirror-cjs.mjs || goto :failed

echo.
echo ^> node tests\shortcut-manager-check.mjs
node tests\shortcut-manager-check.mjs || goto :failed
...
node tests\repository-hygiene-check.mjs || goto :failed

echo.
echo ^> node tests\lint-security.mjs
node tests\lint-security.mjs || goto :failed

echo.
echo ^> node tests\security-smoke.mjs
node tests\security-smoke.mjs || goto :failed

echo.
echo ^> node tests\desktop-host-static-check.mjs
node tests\desktop-host-static-check.mjs || goto :failed
...
powershell.exe -NoLogo -NoProfile -ExecutionPolicy Bypass -File tests\Check-ReleasePackage.ps1 || goto :failed

:releasePackageReady
echo.
echo ^> powershell.exe -NoLogo -NoProfile -ExecutionPolicy Bypass -File tests\Run-BrowserChecks.ps1
powershell.exe -NoLogo -NoProfile -ExecutionPolicy Bypass -File tests\Run-BrowserChecks.ps1 || goto :failed
...
node tests\vendor-audit.mjs || goto :failed

echo.
echo ^> node tools\bundle-prosemirror-cjs.mjs --reuse-bundled-modules --check
node tools\bundle-prosemirror-cjs.mjs --reuse-bundled-modules --check || goto :failed
