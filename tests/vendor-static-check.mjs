...
assert.match(proseMirrorBundle, /function\s+enterLinkHrefEditorAtBoundaryCommand[\s\S]+selection\.from < range\.to - 1[\s\S]+TextSelection\.create\(editorState\.doc, selection\.from \+ 1\)/, 'ArrowRight should move the ProseMirror selection through link text before entering the href editor');
assert.match(proseMirrorBundle, /linkHrefArrowExit = \{ editorView: active\.editorView, pos: active\.range\.to \}/, 'leaving the href editor should allow the next ArrowRight press to continue past the link');
