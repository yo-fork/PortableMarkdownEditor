...
assert.match(app, /async function\s+openSingleMarkdownFile\(file, options = \{\}\)[\s\S]+attachPreviouslyGrantedDirectoryToOpenedMarkdown\(file, options\.fileHandle \|\| null\)[\s\S]+clearPersistedDirectoryHandle\(\)/, 'opening a file should try an already-granted folder before clearing folder access');
assert.match(app, /async function\s+attachPreviouslyGrantedDirectoryToOpenedMarkdown\(file, fileHandle\)[\s\S]+fileHandle\?\.isSameEntry[\s\S]+queryDirectoryPermission\(directoryHandle, 'readwrite'\)[\s\S]+findOpenedMarkdownEntry\(entries, file, fileHandle\)/, 'file open should reuse an existing folder only when the file handle is verified inside that granted folder');
