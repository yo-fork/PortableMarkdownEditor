...
requireFile('vendor/prosemirror/editor-integration.js');
requireFile('tools/bundle-prosemirror-cjs.mjs');
...
const codeMirrorSource = read('vendor/codemirror6/source-editor.js');
...
assert.match(index, /data-format="checklist"[\s\S]+Control\+Alt\+C/, 'toolbar should expose a checklist insertion button and shortcut');
assert.match(index, /data-format="strikethrough"[\s\S]+Control\+Shift\+X/, 'toolbar should expose strikethrough and its shortcut');
...
assert.match(codeMirrorSource, /captureScrollAnchor\(viewportY = 0\)[\s\S]+lineBlockAtHeight\(documentY\)/, 'CodeMirror source should map wrapped visual lines to Markdown offsets');
assert.match(codeMirrorSource, /restoreScrollAnchor\(anchor = \{\}\)[\s\S]+lineBlockAt\(offset\)[\s\S]+block\.height \* sourceProgress/, 'CodeMirror source should restore rendered offsets inside wrapped lines');
assert.match(codeMirrorBundle, /captureScrollAnchor\(viewportY = 0\)[\s\S]+restoreScrollAnchor\(anchor = \{\}\)/, 'the generated CodeMirror bundle should include semantic scroll anchors');
...
const proseMirrorIntegration = read('vendor/prosemirror/editor-integration.js');
assert.match(proseMirrorBundle, /Generated by tools\/bundle-prosemirror-cjs\.mjs/, 'ProseMirror bundle should be reproducibly generated by the local bundler');
assert.ok(proseMirrorBundle.includes(proseMirrorIntegration.trim()), 'ProseMirror bundle should contain the tracked integration source');
assert.match(proseMirrorBundle, /global\.PMEProseMirror/, 'ProseMirror bundle should expose one local global');
assert.match(proseMirrorBundle, /function\s+unsupportedMarkdownReason[\s\S]+markdownEnv\.references/, 'reference definitions should be detected before a lossy rich-edit round trip');
assert.match(proseMirrorBundle, /function\s+markdownTokensContainEmptyLink[\s\S]+link_open[\s\S]+link_close/, 'empty links should be detected before the mark-only rich model can discard them');
assert.match(proseMirrorBundle, /function\s+requiresCanonicalMarkdownNormalization[\s\S]+markdownHasAmbiguousInlineMathBoundary[\s\S]+escapeInlineMathPipesInMarkdownTables/, 'source preservation should retain mandatory canonicalization for ambiguous inline math and table pipes');
...
assert.match(proseMirrorBundle, /function\s+listIsTightIncludingEmptyItems[\s\S]+paragraph_open[\s\S]+!token\.hidden/, 'empty list items should not make an otherwise tight list serialize as loose');
assert.match(proseMirrorBundle, /strikeMarkSpec[\s\S]+parseDOM:[\s\S]+tag: 'del'[\s\S]+append\(\{ strike: strikeMarkSpec \}\)/, 'ProseMirror schema should retain strikethrough as an editable mark');
assert.match(proseMirrorBundle, /tokenizer\.enable\(\['table', 'strikethrough'\]\)[\s\S]+s: \{ mark: 'strike' \}/, 'ProseMirror Markdown parsing should enable strikethrough tokens');
assert.match(proseMirrorBundle, /strike:[\s\S]+open: '~~'[\s\S]+close: '~~'/, 'ProseMirror serialization should emit Markdown strikethrough delimiters');
assert.match(proseMirrorBundle, /text:\s*function\(state, node\)[\s\S]+replace\(\/\\u00a0\/g, '&nbsp;'\)/, 'ProseMirror serialization should preserve non-breaking spaces as stable Markdown entities');
assert.match(proseMirrorBundle, /code_block:\s*function\(state, node\)[\s\S]+if \(source\)[\s\S]+state\.write\(fence\)/, 'empty code blocks should serialize without inventing a content newline');
assert.match(proseMirrorBundle, /function\s+taskListItemInputRule[\s\S]+InputRule\(\/\^\\\[\(\[ xX\]\)\\\]\\s\$\/[\s\S]+setNodeMarkup/, 'typing a task marker inside an auto-created bullet item should promote it to a checklist item');
assert.match(proseMirrorBundle, /function\s+applyChecklistFormatCommand[\s\S]+wrapInList\(schema\.nodes\.bullet_list\)[\s\S]+setChecklistAttrs/, 'the rich checklist toolbar command should wrap paragraphs and set task attrs');
...
assert.match(proseMirrorBundle, /var\s+mathHeadingSpec[\s\S]+content:\s*'\(text \| image \| math_inline\)\*'[\s\S]+update\('heading', mathHeadingSpec\)/, 'ProseMirror headings should explicitly allow inline math nodes without broadening other block content');
...
assert.match(proseMirrorBundle, /if\s+\(isInlineMathSourceEditor\(control\)\)[\s\S]+belowTop\s*=\s*rect\.bottom \+ inlineGap[\s\S]+aboveTop\s*=\s*rect\.top - inlineRect\.height - inlineGap/, 'inline KaTeX source editor should be placed above or below the rendered formula instead of covering it');
...
assert.match(proseMirrorBundle, /function\s+mathSourceEditorValue[\s\S]+return displayMode \? latex : latex \? '\$' \+ latex \+ '\$' :/, 'empty inline KaTeX source editors should use an explicit fallback delimiter');
assert.match(proseMirrorBundle, /function\s+setInlineMathSourceEditorTokens[\s\S]+var tokens = latex \? \['\$', '\$'\] :/, 'the inline math editor should show the same unambiguous delimiters used for an empty formula');
...
assert.match(proseMirrorBundle, /function\s+normalizeInlineMathEditorInput[\s\S]+escapeInlineMath\(latex\)[\s\S]+input\.setSelectionRange\(nextStart, nextEnd\)/, 'typing a literal dollar inside inline math should escape it immediately while preserving the caret');
...
assert.match(proseMirrorBundle, /pme-inline-math-editor-label[\s\S]+this\.sourceEditor\.appendChild\(this\.editPreview\)[\s\S]+bindNodeSourceEditorActivation\(this\.dom, this\.sourceEditor\)/, 'inline KaTeX source editing should keep its label and rendered preview inside the source popover');
assert.match(proseMirrorBundle, /function\s+MathNodeView\(node, editorView, getPos\)[\s\S]+createNodeSourceEditor[\s\S]+normalizeInlineMathEditorInput\(event && event\.target, value\)[\s\S]+selectAfterNode:\s*!self\.displayMode/, 'KaTeX node views should normalize and apply direct rich-mode source edits');
...
assert.match(proseMirrorBundle, /function\s+MathNodeView\(node, editorView, getPos\)[\s\S]+onDeleteBoundary:\s*function\(side\)[\s\S]+if \(!self\.displayMode\)[\s\S]+setSelectionBeforeNodeView[\s\S]+setSelectionAfterNodeView/, 'inline KaTeX source boundaries should exit the editor without deleting the whole formula');
...
assert.match(proseMirrorBundle, /function\s+highlightedCodeRanges[\s\S]+global\.hljs[\s\S]+highlighter\.highlight[\s\S]+result\._emitter/, 'rich code blocks should tokenize selected languages with the vendored Highlight.js runtime');
assert.match(proseMirrorBundle, /function\s+richCodeHighlightDecorations[\s\S]+view\.Decoration\.inline[\s\S]+function\s+richCodeHighlightPlugin/, 'Highlight.js token ranges should be rendered as editable ProseMirror decorations');
assert.match(proseMirrorBundle, /this\.contentDOM\.className\s*=\s*'hljs'[\s\S]+richCodeHighlightPlugin\(\)/, 'rich code blocks should apply the Highlight.js theme and install the highlighting plugin');
...
assert.match(proseMirrorBundle, /inlineMarkInputRule\(\/~~\(\[\^~\\n\]\+\)~~\$\/, schema\.marks\.strike\)/, 'typing Markdown strikethrough should create a strike mark');
assert.match(proseMirrorBundle, /case 'strikethrough': return run\(commands\.toggleMark\(schema\.marks\.strike\)\)/, 'the rich strikethrough action should toggle the strike mark');
...
assert.match(styles, /pme-node-source-editor\.pme-node-source-popover\.pme-node-source-editor--math-inline[\s\S]+background:\s*var\(--panel\);[\s\S]+border-radius:\s*8px;[\s\S]+box-shadow:/, 'inline KaTeX source editor should use an opaque, bounded surface above surrounding text');
...
assert.match(styles, /math-inline\.pme-math-node\.is-editing-source\s*\{[\s\S]+outline:\s*2px solid/, 'inline KaTeX source editing should visibly identify the rendered formula being edited');
assert.match(styles, /math-inline\.pme-math-node\[data-latex=""\][\s\S]+content:\s*"空の数式"/, 'empty inline formulas should remain visible and selectable in rich mode');
assert.doesNotMatch(styles, /math-inline\.pme-math-node\.is-editing-source \.pme-node-rendered-preview[\s\S]+display:\s*none/, 'inline KaTeX source editing should keep the original rendered formula visible');
assert.match(styles, /pme-node-source-editor--math-inline \.pme-inline-math-edit-preview[\s\S]+display:\s*flex;[\s\S]+overflow-x:\s*auto;/, 'inline KaTeX editing should keep its live rendered preview inside the source popover');
...
assert.equal(
  proseMirrorContext.window.PMEProseMirror.normalizeMarkdown('Keep ~~removed text~~ editable.'),
  'Keep ~~removed text~~ editable.',
  'strikethrough should retain its meaning through rich-mode normalization',
);
assert.equal(
...
assert.equal(
  proseMirrorContext.window.PMEProseMirror.normalizeMarkdown(String.raw`空の式 \(\)`),
  String.raw`空の式 \(\)`,
  'an empty inline formula should use a stable delimiter that cannot become display math',
);
const escapedDollarMath = String.raw`価格は $\$$`;
assert.equal(
  proseMirrorContext.window.PMEProseMirror.normalizeMarkdown(escapedDollarMath),
  escapedDollarMath,
  'a literal dollar inside inline math should not gain another backslash during normalization',
);
assert.equal(
  ...
  escapedDollarMath,
  'inline math containing a literal dollar should be idempotent across fresh rich-mode parses',
);
assert.equal(
  proseMirrorContext.window.PMEProseMirror.normalizeMarkdown('$x$$'),
  String.raw`\(x\)$`,
  'a dollar typed immediately after inline math should remain literal text beside the formula',
);
assert.equal(
  proseMirrorContext.window.PMEProseMirror.normalizeMarkdown('$x$$y$'),
  String.raw`\(x\)\(y\)`,
  'adjacent inline formulas should remain two formulas instead of merging into different LaTeX',
);
assert.equal(
  proseMirrorContext.window.PMEProseMirror.normalizeMarkdown('Outside $P(A|B)$ stays raw.'),
  'Outside $P(A|B)$ stays raw.',
  'formula pipes outside tables should remain source-faithful',
);
assert.equal(
  proseMirrorContext.window.PMEProseMirror.normalizeMarkdown('# 物理の見出し $E=mc^2$'),
  '# 物理の見出し $E=mc^2$',
  'inline math should survive rich-mode normalization inside headings',
);
assert.equal(
  proseMirrorContext.window.PMEProseMirror.normalizeMarkdown([
    '| 種別 | 数式 |',
  ...
  'inline math should survive table parsing and escape formula pipes only as Markdown table delimiters',
);
assert.equal(
